...
export default function MissionBuilder({ clinicId, clientUuid, onComplete, onBack, initialDraft }: MissionBuilderProps) {
  ...
  const canGenerate = filledCount >= 7;
    ...
    <div style={{ background: DK.bg, minHeight: "100vh" }}>
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm mt-4 leading-relaxed" style={{ color: DK.textSub }}>
            先生の想い・原点・こだわりを言語化し、<br />
            AIが3パターンの理念を提案します
          </p>
          <p className="text-xs mt-3 mb-5 font-mono" style={{ color: DK.textMuted }}>
            {ALL_MISSION_QUESTIONS.length} questions / approx. 10 min
          </p>
        ...
        {!result && (
          <div className="mb-12 text-center">
            <button onClick={handleGenerate} disabled={!canGenerate || loading}
              ...
              {loading ? "Generating..." : canGenerate ? "Generate 3 Patterns" : `${7 - filledCount} more to go`}
            </button>
            {!canGenerate && <p className="text-xs mt-3" style={{ color: DK.textMuted }}>Minimum 7 answers required</p>}
          </div>
        )}
