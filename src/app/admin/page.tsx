...
function getClinicProgress(clinicId: string): number {
  if (typeof window === "undefined") return 0;
  const saved = loadClinicData(clinicId);
  if (!saved) return 0;
  const allFields = sections.flatMap((s) => s.fields);
  const filled = allFields.filter((f) => saved.data[f.name]?.trim()).length;
...
export default function AdminPage() {
  ...
  // 進捗順にソート
  const sortedClinics = [...clinics].sort((a, b) => getClinicProgress(b.id) - getClinicProgress(a.id));
  ...
  return (
    <main className="px-4 py-8 sm:py-12 max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-medium" style={{ color: "var(--md-on-surface)" }}>クリニック管理</h1>
          <p className="text-xs" style={{ color: "var(--md-on-surface-variant)" }}>
            {clinics.length} 医院 登録済み
          </p>
        </div>
      </div>

      {/* Clinic list with progress ranking */}
      <div className="space-y-2 mb-6">
        {sortedClinics.map((clinic, rank) => {
          const pct = getClinicProgress(clinic.id);
          return (
            <div
              key={clinic.id}
              className="p-4"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-1 shrink-0">
                  <a
                    ...
                    style={{ background: "var(--md-primary)", color: "var(--md-on-primary)", borderRadius: "100px", textDecoration: "none" }}
                  >
                    開く
                  </a>
                  <button
                    ...
                    style={{ background: "transparent", color: "var(--md-error)", borderRadius: "100px", border: "1px solid var(--md-error)", cursor: "pointer" }}
                  >
                    削除
                  </button>
                </div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <div className="flex-1 h-1.5 overflow-hidden" style={{ background: "var(--md-outline-variant)", borderRadius: "100px" }}>
                  <div className="h-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? "var(--md-tertiary)" : "var(--md-primary)", borderRadius: "100px" }} />
                </div>
                <span className="text-xs font-bold" style={{ color: pct === 100 ? "var(--md-tertiary)" : "var(--md-primary)" }}>
                  {pct}%
                </span>
              </div>
            </div>
      ...
      {showForm ? (
        <form onSubmit={handleAdd} className="p-5 space-y-4" style={{ background: "var(--md-surface-container)", borderRadius: "var(--md-shape-corner-xl)", boxShadow: "var(--md-elevation-2)" }}>
          <h3 className="font-medium text-sm" style={{ color: "var(--md-on-surface)" }}>新しいクリニックを追加</h3>
